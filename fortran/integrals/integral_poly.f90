...
! f(t/(1-t²))*(1+t²)/(1-t²)² dt
  ...
  function integrating4(n, m, x)
    ...
    integrating4 = hermitePoly(n%n, x/(1-x**2)) &
      *hermitePoly(m%n, x/(1-x**2)) *((1+x**2)/(1-x**2)**2) &
      *exp(-(x/(1-x**2))**2)
  end function
...
! trapezoidal rule
  ...
  subroutine integralHermite4(n, m)
    ...
    do i=1,ITERATIONS
      total = total +(integrating4(n,m,i*STEP) +integrating4(n,m,(i-1)*STEP)) &
        *STEP/2
      total = total+ (integrating4(n,m,-i*STEP) +integrating4(n,m,-(i-1)*STEP)) &
        *STEP/2
    end do
...
! expresión (5)
  function integrating5(n, x)
    ...
    real :: x, integrating5
    integrating5 = hermitePoly(n%n, x/(1-x**2))**2 /exp((x/(1-x**2))**2) &
      *((1+x**2)/(1-x**2)**2)
  end function
  ...
  subroutine integralHermite5(n)
    ...
    STEP = 0.95/ITERATIONS
    do i=1, ITERATIONS-1
      total = total + integrating5(n, i*STEP)
      total = total + integrating5(n, -i*STEP)
    end do
...
program main
  use auxiliar
  type(parameters) :: n, m ! defined in top of file

  m%n = 4; m%x0 = 0; m%H0 = 12; m%DH0 = 0
  n%n = m%n + 1; n%x0 = 0; n%H0 = 0; n%DH0 = 120
  call integralHermite4(n, m)

  m%n = 7; m%x0 = 0; m%H0 = 0; m%DH0 = -1680;
  n%n = m%n + 1; n%x0 = 0; n%H0 = 1680; n%DH0 = 0;
  call integralHermite4(n, m)

  m%n = 9; m%x0 = 0; m%H0 = 0; m%DH0 = 30240;
  n%n = m%n + 1; n%x0 = 0; n%H0 = -30240; n%DH0 = 0
  call integralHermite4(n, m)

  n%n = 1; n%x0 = 0; n%H0 = 0; n%DH0 = 2
  call integralHermite5(n)

  n%n = 5; n%x0 = 0; n%H0 = 0; n%DH0 = 120
  call integralHermite5(n)

  n%n = 6; n%x0 = 0; n%H0 = -120; n%DH0 = 0
  call integralHermite5(n)
